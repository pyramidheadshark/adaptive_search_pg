...
def calculate_boost(strategy: str, similarity: float, feedback: int) -> float:
    ...
    if strategy == "log":
        return similarity * (1 + 0.1 * math.log(1 + safe_feedback))
    elif strategy == "linear":
        return similarity + (0.002 * safe_feedback)
...
def search_documents(session: Session, query: str, limit: int = 10, strategy: str = "log") -> List[Dict[str, Any]]:
    ...
    stmt = select(Document, (Document.embedding.cosine_distance(query_vector)).label("distance")).order_by(text("distance ASC")).limit(candidates_limit)
    ...
    feedback_stmt = select(Interaction.document_id, func.sum(Interaction.score_delta).label("total_score")).where(Interaction.document_id.in_(doc_ids)).group_by(Interaction.document_id)
